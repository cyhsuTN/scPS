#' Sample Size Calculation for DEA of Multiple Cell Types between Two Independent Groups
...
#' @param low.up.n Lower and upper bounds of total cells per subject in control.
...
#' @param r An allocation ratio (a ratio of no. of subjects in experimental
#' to no. of subjects in control).
#' @param rc A ratio of no. of cells in experimental to in control.
#' @param total A total sample size.
...
#' @export
sizeCal.multi <- function(low.up.m=c(2,10), low.up.n=c(10,100), ePower=0.8, FDR=0.05,
                          grid.m=2, grid.n=10, r=1, rc=1, total=NULL,
                          Genes.tested) {
  ...
  z <- apply(comb.m.n, 1, function(x) {
    ...
    powerCal.multi(ns, ms, Genes.tested, FDR)
  })
  ...
  fig <- ggplot(data=NULL, aes(x=dat2$m, y=dat2$n, fill=dat2$power)) +
    ...
    ylab("No. of total cells per subject in control") +
  ...
  list(m.n.power=dat2, fig=fig, pc=pc, K=K, BA=FALSE, r=r, rc=rc, total=total)
